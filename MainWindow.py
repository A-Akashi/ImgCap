...
class MainWindow:
    ...
    def setUp(self):
        ...
        # TODO
        self.canvas = tk.Canvas(self.window, width=self.camera_controller.vid.get(cv2.CAP_PROP_FRAME_WIDTH),
    ...
    def update(self):
        ...
        if self.camera_controller.isCameraOpen() and ret:
            ...
            self.current_image = self.canvas.create_image(0, 0, image=self.imageTk, anchor=tk.NW)
        self.window.after(10, self.update)

    def exec_Img_Proc(self, frame):
        # 特徴点表示設定時。
        if self.apply_DispFeature:
            frame = self.ImgProc.display_feature_points(frame)
            
        # 物体検知設定時。
        if self.apply_detect and self.reference_image_path:
            frame = self.ImgProc.detect_object(frame, self.reference_image_path)
    ...
    def open_settings(self):
        ...
        self.algorithm_combobox = ttk.Combobox(self.algo_frame, values=["SIFT", "Canny"])
    ...
    def set_auto_wh_on(self):
        if self.auto_wh_var.get() == 0:
            ...
            self.vid.set(cv2.CAP_PROP_AUTO_WB, 1)
